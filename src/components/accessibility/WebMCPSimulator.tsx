...
export function WebMCPSimulator() {
  ...
  const [selectedTool, setSelectedTool] = useState<string>('adapt_reading_view');
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (currentTool.name === 'adapt_reading_view' || currentTool.name === 'adjust_reading_mode') {
        testInput = {
          fontFamily: 'lexend',
      ...
      } else if (currentTool.name === 'activate_rsvp_reader' || currentTool.name === 'control_focal_reader') {
        testInput = {
          action: 'start',
          wpm: 275,
        };
      } else if (currentTool.name === 'render_content_peek' || currentTool.name === 'peek_article') {
        testInput = {
          articleId: 'seed-001',
        };
      } else if (currentTool.name === 'structure_authoring_section' || currentTool.name === 'propose_editor_expansion' || currentTool.name === 'assist_draft_content') {
        testInput = {
          proposal: '## Sensory Accommodations in Modern Workspaces\n\n**Sensory fatigue** is one of the most common yet overlooked barriers for neurodivergent professionals. When soundscapes and lighting remain uncalibrated, working memory collapses within hours.\n\n> "Accommodations are not luxuries; they are fundamental ergonomic adjustments for cognitive preservation."\n\n### Recommended Low-Friction Strategies\n\n- **Brown Noise Masking**: Soft, low-frequency hums eliminate jarring auditory distractions.\n- **Saccade-Free Reading Blocks**: Break deep-work reading into 20-minute visual sprints.\n- **High-Aperture Typefaces**: Prevent character blending during intense document analysis.',
          title: 'Sensory Accommodations for Cognitive Fatigue',
        };
      } else if (currentTool.name === 'insert_interactive_quote' || currentTool.name === 'insert_pull_quote') {
        testInput = {
          quote: "Focus isn't about working harder; it's about removing cognitive friction.",
          attribution: 'Alex M. (NOD Author)',
        };
      } else if (currentTool.name === 'stage_and_publish_post' || currentTool.name === 'publish_article') {
        testInput = {
          title: 'Accessible Agent Authoring: The NOD Experience',
          content: '## Designing for Cognitive Bandwidth\n\nTraditional digital social platforms present **dense walls of text** that trigger severe visual fatigue, character crowding, and reading anxiety for individuals with dyslexia and ADHD.\n\nBy leveraging **WebMCP in-browser tools**, AI agents operate with direct awareness of accessibility typography.\n\n> "Human intent sets the direction; the agent handles cognitive formatting without stripping personal voice."\n\n### Core Tenets of Calm Reading\n\n- **Landmark Headings (##)**: Establish clear spatial anchors every 2-3 paragraphs.\n- **Optical Fixation Points (****)**: Guide saccadic eye movements naturally across the line.\n- **Short Paragraphs**: Never exceed 3 sentences to maintain working memory retention.',
          category: 'strategies',
          tags: ['webmcp', 'accessibility', 'adhd', 'dyslexia'],
        };
      ...
      } else if (currentTool.name === 'search_community_feed' || currentTool.name === 'search_discussions') {
